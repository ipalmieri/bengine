...
void program::setUniform(const GLchar *name, const transMatrix &m, GLboolean transpose) 
{
    GLint uname = uniform(name);
   ...
   if (_object != 0 && uname != -1)
   {
      if (sizeof(tools::Real) == sizeof(double))
      {
	 glProgramUniformMatrix4dv(_object, uname, 1, transpose, (GLdouble *) m.getData());
      }
      else
      {
