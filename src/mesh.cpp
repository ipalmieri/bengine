...
void mesh::setIndexBuffer(mtriangle *buff, const unsigned &count) 
{
   ...
   glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ibuffer);
   glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(mtriangle)*count, buff, GL_STATIC_DRAW);
...
void engine::createCubeMesh(mesh *ms, const float &size)
{
   ...
   for (unsigned i = 0; i < 12; i++)
   {
      ...
      vbuff[ibuff[i].v0].normal[0] = vbuff[ibuff[i].v1].normal[0] = vbuff[ibuff[i].v2].normal[0] = -norm[0];
      vbuff[ibuff[i].v0].normal[1] = vbuff[ibuff[i].v1].normal[1] = vbuff[ibuff[i].v2].normal[1] = -norm[1];
      vbuff[ibuff[i].v0].normal[2] = vbuff[ibuff[i].v1].normal[2] = vbuff[ibuff[i].v2].normal[2] = -norm[2];
...
void engine::createPlaneMesh(mesh *ms, const unsigned &width, const unsigned &length, const float &unit)
{
...
void engine::createSphereMesh(mesh *ms, const float &radius, const unsigned &rings, const unsigned &sectors)
{
   ...
   for (unsigned i = 0; i < rings - 1; i++)
   {
      ...
      {
	 float x = sin(M_PI * ((float) i + 1) * delta_y) * cos(2 * M_PI * ((float) j) * delta_x);
	 float y = sin(M_PI * ((float) i + 1) * delta_y) * sin(2 * M_PI * ((float) j) * delta_x);
