...
renderer::~renderer()
{
   ...
   //delete shaders
   for (vector<shader *>::iterator it = _shaders.begin(); it != _shaders.end(); ++it)
   {
      delete (*it);
   }

   //delete programs
   for (vector<program *>::iterator it = _programs.begin(); it != _programs.end(); ++it)
   {
      delete (*it);
   }

   //delete textures
   for (vector<texture *>::iterator it = _textures.begin();  it != _textures.end(); ++it)
   {
      delete (*it);
   }
...
void renderer::drawScene()
{
   ...
   for (vector<program *>::iterator pg = _programs.begin(); pg != _programs.end(); ++pg)
   {
      program *prog = *pg;
      ...
      for (vector<material *>::iterator mt = _materials.begin(); mt != _materials.end(); ++mt)
      {
...
void renderer::resetScreen(const int &x, const int &y, const unsigned &width, const unsigned &height)
{
   _vport->reset(x, y, width, height);

   frustum *frust = _camera->viewFrustum();

   frust->setPerspective(frust->fieldOfView(), (Real) width / (Real) height, frust->near, frust->far);
}
